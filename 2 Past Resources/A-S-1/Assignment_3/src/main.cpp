...
double ray_sphere_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    double tplus = (-b + sqrt(discriminant)) / (2.0 * a);
    double tminus = (-b - sqrt(discriminant)) / (2.0 * a);

    double t = -1.0;

    // Use the smallest positive t
    if (tplus > 0.0 && (tminus < 0.0 || tplus < tminus)) {
        t = tplus;
    }
    else if (tminus > 0.0) {
        t = tminus;
    ...
    // Check if t is still -1 (no valid intersection)
    if (t < 0.0) {
        return -1.0;
    }
...
bool is_light_visible(const Vector3d &ray_origin, const Vector3d &ray_direction, const Vector3d &light_position)
{
    ...
    Vector3d tmp_p, tmp_N;
    int index = find_nearest_object(ray_origin, ray_direction, tmp_p, tmp_N);
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        Vector3d h = (view_direction + Li).normalized();
        const double specular_coeff = std::pow(fmax(0, (N.dot(h))), obj_specular_exponent);

        // Diffuse contribution
        const Vector4d diffuse = diff_color * std::max(Li.dot(N), 0.0);

        // Specular contribution, use obj_specular_color
        const Vector4d specular = obj_specular_color * specular_coeff;
