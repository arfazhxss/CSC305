...
double ray_sphere_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    Vector3d d = ray_origin-sphere_center; 

    double QA = ray_direction.dot(ray_direction);
    double QB = 2.0 * ray_direction.dot(d); 
    double QC = (d).dot(d) - (sphere_radius * sphere_radius); 
    ...
    double t0 = (-QB - sqrt(discriminant))/(2*QA); // Ray parameter for the first intersection point
    double t1 = (-QB + sqrt(discriminant))/(2*QA); // Ray parameter for the second intersection point
...
{
    // TODO: Determine if the light is visible here.  Cast a ray from each point to the light source, 
    // and check if it intersects any objects before reaching the light source.

    Vector3d p_t, N_t;
    const int nearest_object = find_nearest_object(ray_origin, ray_direction, p_t, N_t);
    if (nearest_object < 0)
    {
        return false;
    }
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        {
            // If the light is not visible, skip this light
            continue;
        }
        ...
        const Vector3d H = (Li+V).normalized();
    ...
    Vector3d r = (d - 2 * (d.dot(N))*N).normalized();


    Vector4d reflection_color(0,0,0,0);
    reflection_color = reflection_color + refl_color.cwiseProduct(shoot_ray(p + (epsilon * r), r, max_bounce - 1));
...
void raytrace_scene()
{
    ...
    double image_y = focal_length*tan(field_of_view/2); // Vertical size of the image plane
    double image_x = aspect_ratio*image_y; // Horizontal size of the image plane
    ...
    for (unsigned i = 0; i < w; ++i)
    {
        for (unsigned j = 0; j < h; ++j)
        {
               ...
               // This implementation was done using an explanation of depth of field from https://pathtracing.home.blog/depth-of-field/
                Vector4d pixel_color(0, 0, 0, 0);
                ...
                //Create a 3d vector of random numbers between 0.5 and -0.5, multiplied by the camera aperture to add to ray origin
                Vector3d random_vector = Vector3d(((double)rand() / (double)RAND_MAX - 0.5) * camera_aperture, ((double)rand() / (double)RAND_MAX - 0.5) * camera_aperture, 0);
                ...
                if (is_perspective)
                {
                    ...
                    //calculate focal point and new ray direction
                    Vector3d focal_point = camera_position + focal_length * ray_direction;
